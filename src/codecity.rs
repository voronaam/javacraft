...
impl Building {
    fn new(class: &Class) -> Building {
        Building {
            ...
            rect: Rect::new(
                class.methods.len() as u16 + 1,
                class.fields.len() as u16 + 1,
            ),
            ...
            match names.split_first() {
                Some((head, tail)) => {
                    self.packages
                        ...
                        .or_insert_with(|| Package::new(head))
                        .add(&tail, class);
                }
                _ => {}
            }
...
pub fn process_class(class: &Class) {
    ...
    for method in &class.methods {
        println!(
            ...
            get_string(&class, method.descriptor_index as usize)
        );
...
fn get_total_code_size(class: &Class) -> usize {
    class
        .methods
        .iter()
        .fold(0usize, |sum, method| sum + get_method_size(method))
}
...
fn get_method_size(method: &Method) -> usize {
    method.attributes.iter().fold(0usize, |sum, a| {
        sum + match a {
...
fn get_method_lines(method: &Method) -> u16 {
    method.attributes.iter().fold(0, |sum, a| {
        sum + match a {
            &Attribute::Code { ref attributes, .. } => attributes.iter().fold(0, |sum, a| {
                sum + match a {
                    &Attribute::LineNumberTable(ref line_table) => {
                        ...
                        if start > end {
                            // TODO: Investigate when it happens
                            0
                        } else {
                            end - start
                        }
...
fn get_code_complexity(code: &Vec<(u32, Instruction)>) -> u16 {
    ...
    code.iter().fold(0, |sum, instruction| {
        sum + match instruction.1 {
...
fn method_music(class: &Class, method: &Method) -> MeasureMeta {
    MeasureMeta {
